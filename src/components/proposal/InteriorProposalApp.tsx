...
export default function InteriorProposalApp() {
  ...
  const buildProposalHtml = () => {
    ...
    return `<!doctype html><html lang="${language}"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width"><title>${escapeHtml(projectName)} ${t('内装提案・見積基礎資料', 'Interior proposal and estimate basis')}</title><style>body{margin:0;background:#eeeae2;color:#242820;font-family:-apple-system,BlinkMacSystemFont,"Noto Sans JP",Arial,sans-serif}.page{max-width:1200px;margin:30px auto;background:white;padding:40px}.top{display:flex;justify-content:space-between;gap:30px;border-bottom:3px solid #263f32;padding-bottom:20px}.eyebrow{font-size:11px;letter-spacing:.15em;color:#587060}h1{margin:6px 0 8px;font-size:28px}.meta{font-size:12px;color:#687067}.summary{text-align:right}.preview{display:block;width:100%;max-height:600px;object-fit:contain;margin:26px 0;border:1px solid #ccd0c9;background:#f3f1eb}.preview.empty{height:280px;display:grid;place-items:center;color:#888}.measurement{display:flex;gap:14px;align-items:baseline;padding:12px 14px;background:#edf4ee;border-left:4px solid #315d44;font-size:11px}.measurement span{font-weight:700}.measurement small{margin-left:auto;color:#667069}h2{font-size:18px;margin:28px 0 10px}table{width:100%;border-collapse:collapse;font-size:10px}th{background:#e4ebe3;text-align:left;padding:8px;border:1px solid #c8cec5}td{padding:7px;border:1px solid #d6d9d3;vertical-align:top}td small{display:block;color:#6e746d;margin-top:3px}.status{font-weight:800;color:#42644e}.swatch{display:block;width:36px;height:30px;border:1px solid #0002;border-radius:3px}.num{text-align:right}.totals{width:380px;margin:18px 0 0 auto}.totals div{display:flex;justify-content:space-between;padding:6px 0;border-bottom:1px solid #ddd}.totals .grand{font-size:18px;font-weight:800;border-top:2px solid #263f32;border-bottom:0;margin-top:4px;padding-top:10px}.pricing{color:#9a612e;font-size:10px;text-align:right;margin-top:8px}.note{margin-top:25px;padding:12px;background:#f4f4ef;font-size:10px;color:#646a63}a{color:#315d44}@media print{body{background:white}.page{margin:0;max-width:none;padding:16px}.preview{max-height:420px}}</style></head><body><main class="page"><header class="top"><div><div class="eyebrow">ASAHI KENSETSU · INTERIOR PROPOSAL POC</div><h1>${escapeHtml(projectName)}</h1><div class="meta">${escapeHtml(customerName)} · ${escapeHtml(room[language === 'ja' ? 'ja' : 'en'])} · ${escapeHtml(designStyle[language === 'ja' ? 'ja' : 'en'])}</div></div><div class="summary"><strong>${t('内装提案・見積基礎資料', 'Interior proposal & estimate basis')}</strong><div class="meta">${generatedAt}</div></div></header>${preview}${measurementSummary}<h2>${t('材料・設備・参考家具明細', 'Materials, fixtures & reference furniture')}</h2><table><thead><tr><th>${t('画像', 'Image')}</th><th>${t('区分・状態', 'Section / status')}</th><th>${t('品名・仕様', 'Description / specification')}</th><th>${t('サイズ', 'Size')}</th><th>${t('色・品番', 'Color / code')}</th><th>${t('メーカー・出典', 'Maker / source')}</th><th>${t('数量', 'Qty')}</th><th>${t('単位', 'Unit')}</th><th>${t('単価', 'Unit price')}</th><th>${t('金額', 'Amount')}</th></tr></thead><tbody>${rows}</tbody></table><div class="totals"><div><span>${t('小計', 'Subtotal')}</span><strong>${currency(subtotal, language)}</strong></div><div><span>${t('消費税 10%', 'Tax 10%')}</span><strong>${currency(tax, language)}</strong></div><div class="grand"><span>${t('合計', 'Total')}</span><strong>${currency(total, language)}</strong></div></div><div class="pricing">${escapeHtml(pricingNote)}</div><div class="note">${t('本書の品番・単価・家具はブランドを使用しないPOCサンプルです。写真からの面積はAI概算であり、正式見積・発注前に現場採寸と実商品データへ置き換えてください。AI画像は完成イメージであり、製品外観を保証しません。', 'SKUs, prices, and furniture in this document are unbranded POC samples. Photo-derived areas are AI estimates; replace them with site measurements and real product data before formal estimating or ordering. AI imagery is conceptual and does not guarantee exact product appearance.')}</div></main></body></html>`;
  };
  ...
  return (
    <div className="studio-app">
      <header className="studio-header">
        <a href="#studio" className="studio-brand"><span>A</span><strong>ASAHI KENSETSU <small>INTERIOR POC</small></strong></a>
      ...
      <main id="studio" className="studio-main">
        <section className="studio-title">
          <div><span>ROOM FINISH STUDIO</span><h1>{t('写真を見ながら、仕上げと参考家具を選ぶ。', 'Choose finishes and reference furniture while viewing the room.')}</h1></div>
